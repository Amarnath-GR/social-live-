...
class RealVideoService {
  ...
  static const List<Map<String, dynamic>> _curatedVideos = [
    {
      ...
      'description': 'Stunning outdoor adventure 🏔️ #adventure #nature',
      'duration': 15000,
      'creator': 'Adventure Pro',
      'tags': ['adventure', 'nature', 'outdoor', 'epic']
    },
    {
      'id': 'sample_reel_2',
      ...
      'title': 'Travel Goals',
      'description': 'Beautiful destinations ✈️ #travel #wanderlust',
      'duration': 15000,
      'creator': 'Travel Vlogger',
      'tags': ['travel', 'destinations', 'wanderlust', 'explore']
    },
    {
      'id': 'sample_reel_3',
      ...
      'title': 'Fun Times',
      'description': 'Having the best time! 🎉 #fun #vibes',
      'duration': 60000,
      'creator': 'Fun Creator',
      'tags': ['fun', 'entertainment', 'vibes', 'party']
    },
    {
      'id': 'sample_reel_4',
      ...
      'title': 'Speed Thrills',
      'description': 'Fast cars and good vibes 🏎️ #cars #speed',
      'duration': 15000,
      'creator': 'Auto Enthusiast',
      'tags': ['cars', 'speed', 'automotive', 'racing']
    },
    {
      'id': 'sample_reel_5',
      ...
      'title': 'Nature Magic',
      'description': 'Mesmerizing natural phenomena ❄️ #nature #magic',
      'duration': 15000,
      'creator': 'Nature Lover',
      'tags': ['nature', 'ice', 'snow', 'beautiful']
    },
    {
      'id': 'big_buck_bunny',
  ...
  static Future<List<VideoModel>> getRealVideos({int limit = 20}) async {
    try {
      // Convert sample reels (first 5) to VideoModel
      final sampleReels = _curatedVideos.take(5).toList().asMap().entries.map((entry) {
      ...
      final remainingVideos = [..._curatedVideos.skip(5), ..._shortFormVideos];
      remainingVideos.shuffle();
      
      final remainingModels = remainingVideos.take(limit - 5).toList().asMap().entries.map((entry) {
        final index = entry.key + 5;
        final videoData = entry.value;
        return VideoModel(
          id: videoData['id'],
          content: videoData['description'],
      ...
      return [...sampleReels, ...remainingModels];
    } catch (e) {
  ...
  static Future<List<VideoModel>> _getFollowedUsersVideos() async {
    try {
      await FollowingService().loadFollowing();
      final followedUsers = FollowingService().followingUsers;
      
      final followedVideos = <VideoModel>[];
      
      for (int i = 0; i < followedUsers.length && i < 5; i++) {
        final user = followedUsers[i];
        final videoData = _curatedVideos[i % _curatedVideos.length];
        
        final video = VideoModel(
          id: 'followed_${user['username']}_$i',
          content: '${videoData['description']} from your followed creator! ${_generateHashtags(videoData['tags'])}',
          videoUrl: videoData['url'],
          thumbnailUrl: videoData['thumbnail'],
          duration: videoData['duration'],
          createdAt: DateTime.now().subtract(Duration(minutes: i * 30)),
          user: UserModel(
            id: 'followed_${user['username']}',
            email: '${user['username']}@example.com',
            username: user['username']!,
            name: user['displayName']!,
            avatar: 'https://via.placeholder.com/100x100/4ECDC4/FFFFFF?text=${user['displayName']![0]}',
            role: 'CREATOR',
            verified: i % 3 == 0,
            isBlocked: false,
      ...
      print('Error generating followed users videos: $e');
      return [];
    }
  }
