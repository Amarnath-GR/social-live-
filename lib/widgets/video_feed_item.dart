...
class VideoFeedItemState extends State<VideoFeedItem> {
  ...
  void _navigateToProfile(BuildContext context) {
    ButtonActionsService.openProfile(context, widget.video['user']?['username'] ?? 'Unknown');
  }

  void _shareVideo() {
    final content = widget.video['content'] ?? '';
    final username = widget.video['user']?['username'] ?? 'Unknown';
    ButtonActionsService.shareContent('Check out this video by @$username: $content\n\n#SocialLiveMVP');
  }
            ...
            children: [
              ...
              AnimatedLikeButton(
                ...
                onTap: () {
                  ...
                  final videoUrl = widget.video['mediaUrl'] ?? '';
                  final title = widget.video['content'] ?? 'Video';
                  final creator = widget.video['user']?['username'] ?? 'Unknown';
                  final currentLikes = widget.video['likesCount'] ?? 0;
                  
                  if (!isLiked) {
                    widget.video['isLiked'] = true;
                    widget.video['likesCount'] = currentLikes + 1;
                    ...
                    widget.video['likesCount'] = currentLikes - 1;
                    LikedVideosService().unlikeVideo(videoId);
                  }
