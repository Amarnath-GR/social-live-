...
class _EnhancedSimpleCameraScreenState extends State<EnhancedSimpleCameraScreen> {
                ...
                Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      icon: Icon(Icons.auto_fix_high, color: Colors.white, size: 28),
                    ),
                  ],
                ),
              ],
  ...
  Future<void> _takePhoto() async {
    ...
    if (_cameraController != null && _cameraController!.value.isInitialized) {
      try {
        ...
        // Fallback for mock
        photoPath = 'photo_${DateTime.now().millisecondsSinceEpoch}';
      }
    } else {
      // Mock camera
      photoPath = 'photo_${DateTime.now().millisecondsSinceEpoch}';
    ...
    // Navigate to preview screen instead of posting directly
    Navigator.push(
      context,
      MaterialPageRoute(
  ...
  void _startLiveStream() {
    // Save live stream to user content
    final streamId = 'live_${DateTime.now().millisecondsSinceEpoch}';
    UserContentService().addLiveStream(streamId);
